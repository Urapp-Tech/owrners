    ...
    (function ($) {
        ...
        $(document).ready(function () {
            ...
            $(document).on('keyup', '#project_title , #slug', function (e) {
                $('.full-slug-show').show();
                let slug = convertToSlug( "You will " + $(this).val());
                $('#slug').val(slug);
        ...
        $(document).on("click", "#next",async function (e){
            ...
            if (current <= Listings.length) {
                ...
                if(current == 1){
                    ...
                    var resp = await $.ajax({
                        url: "{{ route('freelancer.project.validate.intro') }}",
                        type: 'POST',
                        accept: 'application/json',
                        data: {project_title: title, project_description: description,slug: $('#slug').val() , category: category, subcategory: subcategory},
                    ...
                    const file = $('#upload_project_photo')[0].files[0];

                    var fd = new FormData();
                    fd.append('image',file);

                    var resp = await $.ajax({
                        url: "{{ route('freelancer.project.validate.gallery') }}",
                        type: 'POST',
                        accept: 'application/json',
                        data: fd,
                        cache:false,
