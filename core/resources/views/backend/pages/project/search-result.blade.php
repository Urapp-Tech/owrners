            ...
            <td>
                ...
                <ul class="dropdown-menu status_dropdown__list">
                    ...
                    <li class="status_dropdown__item">
                        <a href="{{ route('admin.project.details',$project->id) }}" class="btn dropdown-item status_dropdown__list__link">{{ __('Gig Details') }}</a>
                    </li>
                    @endcan
                    @can('project-delete')
                    <li class="status_dropdown__item">
                        <x-popup.delete-popup :title="__('Delete Gig')" :url="route('admin.project.delete',$project->id)"/>
                    </li>
                    @endcan
                    @can('project-status-change')
                    <li class="status_dropdown__item">
                        @if($project->project_approve_request === 0 || $project->project_approve_request === 2)
                            <x-status.table.status-change :title="__('Activate Gig')" :url="route('admin.project.status.change',$project->id)"/>
                        @else
                            <x-status.table.status-change :title="__('Inactivate Gig')" :url="route('admin.project.status.change',$project->id)"/>
                        @endif
                    </li>
                    @endcan
                </ul>
